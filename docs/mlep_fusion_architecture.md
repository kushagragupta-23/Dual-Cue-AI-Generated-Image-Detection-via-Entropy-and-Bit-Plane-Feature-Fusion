...
Given an input image tensor $X \in \mathbb{R}^{B \times C \times H \times W}$, MLEP first divides the image into non-overlapping Macro-Windows of size $M \times M$. 

Within each Macro-Window, the pixels are further subdivided into Micro-Patches of size $L \times L$. 
A pseudo-random permutation $\pi$ is applied to the Micro-Patches strictly *within* their respective Macro-Windows.
...
The shuffled tensor $X_{shuffled}$ is processed through a Convolutional Neural Network (CNN) backbone (e.g., ResNet-18). We extract intermediate feature maps at three distinct scales to capture both fine-grained texture anomalies and coarse semantic flaws:
...
$$ H(i,j) = - \sum_{d=1}^{D} p_d(i,j) \log_2 p_d(i,j) $$
...
### Mechanism
1. **MLEP Branch**: Processes the shuffled, global image to output a deep feature vector $v_{mlep}$.
2. **Local Branch**: Processes isolated high-frequency noise patches to output a deep feature vector $v_{local}$.
3. **Cross-Attention**: A learned multi-layer perceptron (MLP) analyzes both vectors and predicts a dynamic gating weight $\alpha \in [0, 1]$.
4. **Weighted Fusion**: The final classification vector is a convex combination of both cues:
   
   $$ v_{final} = \alpha \cdot v_{mlep} + (1 - \alpha) \cdot v_{local} $$
